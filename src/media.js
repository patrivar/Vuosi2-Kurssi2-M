...
const postNewMediaItem = (req, res) => {
  ...
  res.status(201).json({ message: "media item created", item: data });
};

/**
 * Deletes media item from mockdata based on value of id
...
const deleteMediaByID = (req, res) => {
  ...
  if (itemToBeDeletedIndex != -1) {
    res.status(204).json({ message: "media item deleted" });
  } else {
    res.status(404).json({ message: "media not found" });
  }
};
